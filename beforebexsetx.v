...
module processor(
	...
	// Global pipeline stall (declared early: used by register `we` ports throughout the module)
	wire stall;

	// ================FETCH STAGE=================== //
    wire [31:0] pc_in, pc_out, pc_plus_one, f_d_pc_out,f_d_ir_out;
    //pc = pc+1
    register pc_reg(.clk(clock), .in(pc_in), .out(pc_out), .clrn(reset), .we(!stall)); // latch to hold current program counter
    ...
    // ================FETCH-DECODE=================== //
    register fd_ir_latch(.clk(~clock), .in(q_imem), .out(f_d_ir_out), .clrn(reset), .we(!stall));//clk, in, out, clrn, we
    register fd_pc_latch(.clk(~clock), .in(pc_out), .out(f_d_pc_out), .clrn(reset), .we(!stall));
    ...
    wire is_multdiv_d, is_multdiv_x, is_bex, is_setx;
    ...
    assign ctrl_readRegA = is_bex ?  5'd30 : f_d_ir_out[21:17]; // $rs
    ...
    assign is_bex = f_d_ir_out[31:27] == 5'b10110;
    assign is_setx = f_d_ir_out[31:27] == 5'b10101;
//assign ctrl_readRegB = instruction_type[0] ? f_d_latch_out[16:12] : 5'b0; // $rd, doesn't matter if instruction isn't R type
    
    // ================DECODE-EXECUTE=================== //
    register data_operand_A_latch(.clk(~clock), .in(data_readRegA), .out(d_x_A), .clrn(reset), .we(!stall));
    register data_operand_B_latch(.clk(~clock), .in(data_readRegB), .out(d_x_B), .clrn(reset), .we(!stall));
    register dx_ir1(.clk(~clock), .in(f_d_ir_out), .out(d_x_ir), .clrn(reset), .we(!stall));
    register dx_pc1(.clk(~clock), .in (f_d_pc_out), .out(d_x_pc), .clrn(reset), .we(!stall));
    register dx_inst_type(.clk(~clock), .in({28'd0, instruction_type}), .out(d_x_inst_type), .clrn(reset), .we(!stall));
    dffe_ref md1(.q(is_multdiv_x), .d(is_multdiv_d), .clk(~clock), .en(!stall), .clr(1'b0));
    ...
    dffe_ref pooop(pooop_out, ((d_x_ir[6:2] == 5'b00110) || (d_x_ir[6:2]== 5'b00111)) && d_x_inst_type[0] && !mult_div_completed, clock, 1'b1, 1'b0);
    ...
    // Freeze the whole pipeline while a mult/div instruction is in EX and not yet done.
    // multdiv itself is wired directly to `clock` (not gated by `we`), so its internal
    // counter keeps running during the freeze and can still reach completion.
    wire mult_div_pending = d_x_inst_type[0] && ((d_x_ir[6:2] == 5'b00110) || (d_x_ir[6:2] == 5'b00111));
    assign stall = mult_div_pending && !mult_div_completed;
    
    // ================P-Writeback=================== //
    register multdiv_result_latch(.clk(~clock), .in(mult_div_result), .out(p_w_multdiv_out), .clrn(reset), .we(mult_div_completed)); // only load a new result when one is actually ready; holds otherwise
    register pw_ir1(.clk(~clock), .in(d_x_ir), .out(p_w_ir), .clrn(reset), .we(mult_div_completed));

    // ================EXECUTE-MEMORY=================== //
    register alu_result_latch(.clk(~clock), .in(alu_out), .out(x_m_alu_out), .clrn(reset), .we(!stall));
    register xm_ir1(.clk(~clock), .in(d_x_ir), .out(x_m_ir), .clrn(reset), .we(!stall));
    register b_latch(.clk(~clock), .in(d_x_B), .out(x_m_B), .clrn(reset), .we(!stall));
    register xm_inst_type(.clk(~clock), .in(d_x_inst_type), .out(x_m_inst_type), .clrn(reset), .we(!stall));
    dffe_ref md2(.q(is_multdiv_m), .d(is_multdiv_x), .clk(~clock), .en(!stall), .clr(1'b0));
    ...
    // ================MEMORY-WRITEBACK=================== //
    register alu_result_m_w_latch(.clk(~clock), .in(x_m_alu_out), .out(m_w_alu_out), .clrn(reset), .we(!stall));
    register data_memory_latch(.clk(~clock), .in(q_dmem), .out(data_memory_out), .clrn(reset), .we(!stall));
    register mw_ir1(.clk(~clock), .in(x_m_ir), .out(m_w_ir), .clrn(reset), .we(!stall));
    register multdiv_result_w_latch(.clk(~clock), .in(p_w_multdiv_out), .out(w_multdiv_out), .clrn(reset), .we(!stall));
    dffe_ref md3(.q(is_multdiv_w), .d(is_multdiv_m), .clk(~clock), .en(!stall), .clr(1'b0));
